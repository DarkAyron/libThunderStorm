/*****************************************************************************/
/* SListFile.c                            Copyright (c) Ladislav Zezula 2004 */
/*---------------------------------------------------------------------------*/
/* Description:                                                              */
/*---------------------------------------------------------------------------*/
...
/* 31.03.15  1.00  Ayr  Ported to plain c                                    */
/*****************************************************************************/
...
{
    void *  hFile;                      /* Stormlib file handle */
    char  * szMask;                     /* Self-relative pointer to file mask */
    uint32_t   dwFileSize;                 /* Total size of the cached file */
    uint32_t   dwFilePos;                  /* Position of the cache in the file */
    uint8_t  * pBegin;                     /* The begin of the listfile cache */
    uint8_t  * pPos;
    uint8_t  * pEnd;                       /* The last character in the file cache */
...
static int FreeListFileCache(struct TListFileCache * pCache)
{
...
static struct TListFileCache * CreateListFileCache(void * hListFile, const char * szMask)
{
    ...
    pCache = (struct TListFileCache *)STORM_ALLOC(uint8_t, sizeof(struct TListFileCache) + nMaskLength);
    if(pCache != NULL)
    {
        /* Clear the entire structure */
        memset(pCache, 0, sizeof(struct TListFileCache) + nMaskLength);
        ...
        SFileReadFile(hListFile, pCache->Buffer, CACHE_BUFFER_SIZE, &dwBytesRead);
        if(dwBytesRead != 0)
        {
            /* Allocate pointers */
...
static uint32_t ReloadListFileCache(struct TListFileCache * pCache)
{
    ...
    if(pCache->pPos >= pCache->pEnd)
    {
        ...
        SFileSetFilePointer(pCache->hFile, pCache->dwFilePos, NULL, SEEK_SET);
        SFileReadFile(pCache->hFile, pCache->Buffer, dwBytesToRead, &dwBytesRead);
...
static size_t ReadListFileLine(struct TListFileCache * pCache, char * szLine, int nMaxChars)
{
...
static int CompareFileNodes(const void * p1, const void * p2) 
{
    ...
    return strcasecmp(szFileName1, szFileName2);
}

static unsigned char * CreateListFile(TMPQArchive * ha, uint32_t * pcbListFile)
{
    ...
    if(nFileNodes > 0)
    {
        ...
        for(nIndex0 = 0, nIndex1 = 1; nIndex1 < nFileNodes; nIndex1++)
        {
            ...
            if(strcasecmp(SortTable[nIndex1], SortTable[nIndex0]) != 0)
            {
    ...
    if(pcbListFile != NULL)
        *pcbListFile = (uint32_t)cbListFile;
    return (unsigned char *)szListFile;
}
...
{
    TMPQFile * hf = NULL;
    unsigned char * pbListFile;
    uint32_t cbListFile = 0;
    ...
    void * hMpq)
{
    ...
    TMPQHash * pHash;
    void * hListFile;
    uint32_t lcSaveLocale = lcFileLocale;
    int nError = ERROR_SUCCESS;
...
int EXPORT_SYMBOL SFileAddListFile(void * hMpq, const char * szListFile)
{
...
void EXPORT_SYMBOL * SListFileFindFirstFile(void * hMpq, const char * szListFile, const char * szMask, SFILE_FIND_DATA * lpFindFileData)
{
    struct TListFileCache * pCache = NULL;
    void * hListFile = NULL;
    size_t nLength = 0;
    uint32_t dwSearchScope = SFILE_OPEN_LOCAL_FILE;
    int nError = ERROR_SUCCESS;
...
int EXPORT_SYMBOL SListFileFindNextFile(void * hFind, SFILE_FIND_DATA * lpFindFileData)
{
    struct TListFileCache * pCache = (struct TListFileCache *)hFind;
    size_t nLength;
...
int EXPORT_SYMBOL SListFileFindClose(void * hFind)
{
    struct TListFileCache * pCache = (struct TListFileCache *)hFind;
