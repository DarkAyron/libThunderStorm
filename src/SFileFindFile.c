/*****************************************************************************/
/* SFileFindFile.c                        Copyright (c) Ladislav Zezula 2003 */
/*---------------------------------------------------------------------------*/
...
typedef struct 
{
    ...
    uint32_t  dwSearchTableItems;          /* Number of items in the search table */
    uint32_t  dwNextIndex;                 /* Next file index to be checked */
    uint32_t  dwFlagMask;                  /* For checking flag mask */
...
static TMPQSearch * IsValidSearchHandle(void * hFind)
{
    ...
    for(;;)
    {
        ...
        if(*szWildCard == '*')
        {
            ...
            while(*szString != 0)
            {
                ...
                {
                    if(AsciiToUpperTable[(uint8_t)szString[nMatchCount]] != AsciiToUpperTable[(uint8_t)szWildCard[nMatchCount]])
                        break;
        ...
        else
        {
            ...
            if(AsciiToUpperTable[(uint8_t)*szString] != AsciiToUpperTable[(uint8_t)*szWildCard])
                return 0;
...
static uint32_t GetSearchTableItems(TMPQArchive * ha)
{
    uint32_t dwMergeItems = 0;
    ...
    TFileEntry * pFileEntry)
{
    ...
    if(hs->pSearchTable != NULL && szRealFileName != NULL)
    {
        ...
        if(ha->pPatchPrefix != NULL)
        {
            ...
            if(strncasecmp(szRealFileName, ha->pPatchPrefix->szPatchPrefix, ha->pPatchPrefix->nLength))
                return 1;
        ...
        if(ha->haBase != NULL)
        {
            ...
            for(;;)
            {
                ...
                {
                    /* Does the name match? */
                    if(!strcasecmp(pEntry->szFileName, szRealFileName))
                        return 1;
                }
...
static TFileEntry * FindPatchEntry(TMPQArchive * ha, TFileEntry * pFileEntry)
{
    ...
    uint32_t lcLocale = pFileEntry->lcLocale;
    ...
    while(ha != NULL)
    {
        ...
        while(pFileEntry < pFileTableEnd)
        {
            ...
            if((pFileEntry->dwFlags & hs->dwFlagMask) == MPQ_FILE_EXISTS)
            {
                ...
                {
/*                  if(pFileEntry != NULL && !strcasecmp(pFileEntry->szFileName, "TriggerLibs\\NativeLib.galaxy")) */
/*                      DebugBreak(); */
                    ...
                    dwBlockIndex = (uint32_t)(pFileEntry - ha->pFileTable);
                    ...
                    if(szFileName == NULL)
                    {
                        ...
                        if(SFileOpenFileEx((void *)hs->ha, szPseudoName, SFILE_OPEN_BASE_FILE, &hFile))
                        {
                    ...
                    if(szFileName != NULL)
                    {
                        ...
                        if(CheckWildCard(szFileName + nPrefixLength, hs->szSearchMask))
                        {
                            ...
                            /* Fill the filetime */
                            lpFindFileData->dwFileTimeHi = (uint32_t)(pPatchEntry->FileTime >> 32);
                            lpFindFileData->dwFileTimeLo = (uint32_t)(pPatchEntry->FileTime);
...
void EXPORT_SYMBOL * SFileFindFirstFile(void * hMpq, const char * szMask, SFILE_FIND_DATA * lpFindFileData, const char * szListFile)
{
...
int EXPORT_SYMBOL SFileFindNextFile(void * hFind, SFILE_FIND_DATA * lpFindFileData)
{
...
int EXPORT_SYMBOL SFileFindClose(void * hFind)
{
    TMPQSearch * hs = IsValidSearchHandle(hFind);
