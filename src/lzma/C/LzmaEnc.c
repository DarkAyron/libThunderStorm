...
void LzmaEnc_FastPosInit(uint8_t *g_FastPos)
{
  ...
  for (slotFast = 2; slotFast < kNumLogBits * 2; slotFast++)
  {
    ...
    uint32_t j;
    for (j = 0; j < k; j++, c++)
      g_FastPos[c] = (unsigned char)slotFast;
  }
}
...
static void LenEnc_Encode2(CLenPriceEnc *p, CRangeEnc *rc, uint32_t symbol, uint32_t posState, int updatePrice, uint32_t *ProbPrices)
{
...
static int LzmaEnc_CodeOneBlock(CLzmaEnc *p, int useLimits, uint32_t maxPackSize, uint32_t maxUnpackSize)
{
...
int LzmaEnc_PrepareForLzma2(CLzmaEncHandle pp,
...
int LzmaEnc_MemPrepare(CLzmaEncHandle pp, const uint8_t *src, size_t srcLen,
...
int LzmaEnc_CodeOneMemBlock(CLzmaEncHandle pp, int reInit,
...
int LzmaEnc_Encode(CLzmaEncHandle pp, ISeqOutStream *outStream, ISeqInStream *inStream, ICompressProgress *progress,
    ISzAlloc *alloc, ISzAlloc *allocBig)
{
...
int LzmaEncode(uint8_t *dest, size_t *destLen, const uint8_t *src, size_t srcLen,
