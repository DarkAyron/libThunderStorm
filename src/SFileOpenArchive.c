/*****************************************************************************/
/* SFileOpenArchive.c                         Copyright Ladislav Zezula 1999 */
/*                                                                           */
...
static int IsAviFile(uint32_t * HeaderData)
{
    uint32_t DwordValue0 = BSWAP_INT32_UNSIGNED(HeaderData[0]);
    uint32_t DwordValue2 = BSWAP_INT32_UNSIGNED(HeaderData[2]);
    uint32_t DwordValue3 = BSWAP_INT32_UNSIGNED(HeaderData[3]);

    /* Test for 'RIFF', 'AVI ' or 'LIST' */
    return (DwordValue0 == 0x46464952 && DwordValue2 == 0x20495641 && DwordValue3 == 0x5453494C);
}

static int IsWarcraft3Map(uint32_t * HeaderData)
{
    uint32_t DwordValue0 = BSWAP_INT32_UNSIGNED(HeaderData[0]);
    uint32_t DwordValue1 = BSWAP_INT32_UNSIGNED(HeaderData[1]);

    return (DwordValue0 == 0x57334D48 && DwordValue1 == 0x00000000);
}

static TMPQUserData * IsValidMpqUserData(uint64_t ByteOffset, uint64_t FileSize, void * pvUserData)
{
...
static int VerifyMpqTablePositions(TMPQArchive * ha, uint64_t FileSize)
{
...
uint32_t EXPORT_SYMBOL SFileGetLocale()
{
    return lcFileLocale;
}

uint32_t EXPORT_SYMBOL SFileSetLocale(uint32_t lcNewLocale)
{
...
int EXPORT_SYMBOL SFileOpenArchive(
    ...
    void * * phMpq)
{
    ...
    uint64_t FileSize = 0;             /* Size of the file */
    unsigned char * pbHeaderBuffer = NULL;       /* Buffer for searching MPQ header */
    int bIsWarcraft3Map = 0;
    ...
    {
        uint32_t dwStreamFlags = (dwFlags & STREAM_FLAGS_MASK);
    ...
    if(nError == ERROR_SUCCESS)
    {
        pbHeaderBuffer = STORM_ALLOC(uint8_t, HEADER_SEARCH_BUFFER_SIZE);
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        uint64_t EndOfSearch = FileSize;
        uint32_t dwStreamFlags = 0;
        ...
        while(bSearchComplete == 0 && SearchOffset < EndOfSearch)
        {
            /* Always read at least 0x1000 bytes for performance. */
            /* This is what Storm.dll (2002) does. */
            uint32_t dwBytesAvailable = HEADER_SEARCH_BUFFER_SIZE;
            uint32_t dwInBufferOffset = 0;

            /* Cut the bytes available, if needed */
            if((FileSize - SearchOffset) < HEADER_SEARCH_BUFFER_SIZE)
                dwBytesAvailable = (uint32_t)(FileSize - SearchOffset);
            ...
            if(SearchOffset == 0)
            {
                if(IsAviFile((uint32_t *)pbHeaderBuffer))
                {
                    nError = ERROR_AVI_FILE;
                    break;
                }

                bIsWarcraft3Map = IsWarcraft3Map((uint32_t *)pbHeaderBuffer);
            }
            ...
            while(dwInBufferOffset < dwBytesAvailable)
            {
                ...
                if(bIsWarcraft3Map == 0 && (dwFlags & MPQ_OPEN_FORCE_MPQ_V1) == 0)
                {
...
/*-----------------------------------------------------------------------------
 * int SFileSetDownloadCallback(void *, SFILE_DOWNLOAD_CALLBACK, void *);
 *
 * Sets a callback that is called when content is downloaded from the master MPQ
 */

int EXPORT_SYMBOL SFileSetDownloadCallback(void * hMpq, SFILE_DOWNLOAD_CALLBACK DownloadCB, void * pvUserData)
{
    TMPQArchive * ha = (TMPQArchive *)hMpq;
...
int EXPORT_SYMBOL SFileCloseArchive(void * hMpq)
{
