/*****************************************************************************/
/* SCompression.c                         Copyright (c) Ladislav Zezula 2003 */
/*---------------------------------------------------------------------------*/
...
/* 23.03.15  1.00  Ayr  Ported to plain c                                    */
/*****************************************************************************/
...
void Compress_huff(void * pvOutBuffer, int * pcbOutBuffer, void * pvInBuffer, int cbInBuffer, int * pCmpType, int nCmpLevel)
{
    ...
    huffTree_init(&ht, 1);
    huff_TOutputStreamInitialise (&os, pvOutBuffer, *pcbOutBuffer);

    STORMLIB_UNUSED(nCmpLevel);
    *pcbOutBuffer = huff_Compress(&ht, &os, pvInBuffer, cbInBuffer, *pCmpType);
}                 
...
int Decompress_huff(void * pvOutBuffer, int * pcbOutBuffer, void * pvInBuffer, int cbInBuffer)
{
    ...
    huffTree_init(&ht, 0);
    huff_TInputStreamInitialise(&is, pvInBuffer, cbInBuffer);
    
    *pcbOutBuffer = huff_Decompress(&ht, pvOutBuffer, *pcbOutBuffer, &is);
...
static int LZMA_Callback_Progress(void * p, uint64_t inSize, uint64_t outSize)
{
    return SZ_OK;
}
...
{
    p = p;
    return STORM_ALLOC(uint8_t, size);
}
...
static void Compress_LZMA(void * pvOutBuffer, int * pcbOutBuffer, void * pvInBuffer, int cbInBuffer, int * pCmpType, int nCmpLevel)
{
    ...
    if(destLen >= (size_t)(*pcbOutBuffer - LZMA_HEADER_SIZE))
        return;
...
static int Decompress_LZMA_MPK(void * pvOutBuffer, int * pcbOutBuffer, void * pvInBuffer, int cbInBuffer)
{
    ...
    size_t srcLen = cbInBuffer;
    int nResult;
    uint8_t LZMA_Props[] = {0x5D, 0x00, 0x00, 0x00, 0x01};
...
int EXPORT_SYMBOL SCompImplode(void * pvOutBuffer, int * pcbOutBuffer, void * pvInBuffer, int cbInBuffer)
{
    int cbOutBuffer;
...
int EXPORT_SYMBOL SCompExplode(void * pvOutBuffer, int * pcbOutBuffer, void * pvInBuffer, int cbInBuffer)
{
    int cbOutBuffer;
...
int EXPORT_SYMBOL SCompCompress(void * pvOutBuffer, int * pcbOutBuffer, void * pvInBuffer, int cbInBuffer, unsigned uCompressionMask, int nCmpType, int nCmpLevel)
{
    ...
    else
    {
        ...
        for(i = 0; i < (sizeof(cmp_table) / sizeof(TCompressTable)); i++)
        {
...
int EXPORT_SYMBOL SCompDecompress(void * pvOutBuffer, int * pcbOutBuffer, void * pvInBuffer, int cbInBuffer)
{
    ...
    for(i = 0; i < (sizeof(dcmp_table) / sizeof(TDecompressTable)); i++)
    {
        /* If the mask agrees, insert the compression function to the array */
...
int EXPORT_SYMBOL SCompDecompress2(void * pvOutBuffer, int * pcbOutBuffer, void * pvInBuffer, int cbInBuffer)
{
...
int EXPORT_SYMBOL SCompDecompressMpk(void * pvOutBuffer, int * pcbOutBuffer, void * pvInBuffer, int cbInBuffer)
{
