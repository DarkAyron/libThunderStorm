/*****************************************************************************/
/* SFileCreateArchive.c                   Copyright (c) Ladislav Zezula 2003 */
/*---------------------------------------------------------------------------*/
...
static const uint32_t MpqHeaderSizes[] =
{
    MPQ_HEADER_SIZE_V1,
    MPQ_HEADER_SIZE_V2,
    MPQ_HEADER_SIZE_V3,
...
static uint16_t GetSectorSizeShift(uint32_t dwSectorSize)
{
    uint16_t wSectorSizeShift = 0;
...
static int WriteNakedMPQHeader(TMPQArchive * ha)
{
    ...
    TMPQHeader Header;
    uint32_t dwBytesToWrite = pHeader->dwHeaderSize;
...
int EXPORT_SYMBOL SFileCreateArchive(const char * szMpqName, uint32_t dwCreateFlags, uint32_t dwMaxFileCount, void * * phMpq)
{
...
int EXPORT_SYMBOL SFileCreateArchive2(const char * szMpqName, PSFILE_CREATE_MPQ pCreateInfo, void ** phMpq)
{
    TFileStream * pStream = NULL;           /* File stream */
    TMPQArchive * ha = NULL;                /* MPQ archive handle */
    TMPQHeader * pHeader;
    uint64_t MpqPos = 0;                   /* Position of MPQ header in the file */
    void * hMpq = NULL;
    uint32_t dwBlockTableSize = 0;             /* Initial block table size */
    uint32_t dwHashTableSize = 0;
    uint32_t dwReservedFiles = 0;              /* Number of reserved file entries */
    uint32_t dwMpqFlags = 0;
    int nError = ERROR_SUCCESS;
    ...
    MpqPos = (MpqPos + 0x1FF) & (uint64_t)0xFFFFFFFFFFFFFE00ULL;
...
/*  dwBlockTableSize = dwHashTableSize * 2; */
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        pHeader->wFormatVersion   = (uint16_t)pCreateInfo->dwMpqVersion;
