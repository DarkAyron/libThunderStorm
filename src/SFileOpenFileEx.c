...
static uint32_t FindHashIndex(TMPQArchive * ha, uint32_t dwFileIndex)
{
    TMPQHash * pHashTableEnd;
    TMPQHash * pHash;
    uint32_t dwFirstIndex = HASH_ENTRY_FREE;
    ...
    for(pHash = ha->pHashTable; pHash < pHashTableEnd; pHash++)
    {
        if(pHash->dwBlockIndex == dwFileIndex)
        {
            ...
            dwFirstIndex = (uint32_t)(pHash - ha->pHashTable);
        }
    }
...
static int OpenLocalFile(const char * szFileName, void ** PtrFile)
{
    TFileStream * pStream;
    TMPQFile * hf = NULL;
...
int OpenPatchedFile(void * hMpq, const char * szFileName, void ** PtrFile)
{
...
int EXPORT_SYMBOL SFileOpenFileEx(void * hMpq, const char * szFileName, uint32_t dwSearchScope, void ** PtrFile)
{
