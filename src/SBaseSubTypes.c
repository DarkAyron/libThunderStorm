/*****************************************************************************/
/* SBaseSubTypes.c                        Copyright (c) Ladislav Zezula 2013 */
/*---------------------------------------------------------------------------*/
...
void * LoadSqpTable(TMPQArchive * ha, uint32_t dwByteOffset, uint32_t cbTableSize, uint32_t dwKey)
{
    uint64_t ByteOffset;
    unsigned char * pbSqpTable;

    /* Allocate buffer for the table */
    pbSqpTable = STORM_ALLOC(uint8_t, cbTableSize);
    if(pbSqpTable != NULL)
    {
        /* Load the table */
...
typedef struct _TMPKHeader
{
    ...
    uint32_t dwBlockTablePos;
    uint32_t dwBlockTableSize;
    uint32_t dwUnknownPos;
    uint32_t dwUnknownSize;
} TMPKHeader;
...
{
    uint32_t  dwFlags;         /* 0x1121 - Compressed , 0x1120 - Not compressed */
    uint32_t  dwFilePos;       /* Offset of the beginning of the file, relative to the beginning of the archive. */
    uint32_t  dwFSize;         /* Uncompressed file size */
    uint32_t  dwCSize;         /* Compressed file size */
    uint32_t  dwUnknown;       /* 0x86364E6D */
} TMPKBlock;
...
/*  Header.dwUnknownSize = BSWAP_INT32_UNSIGNED(pMpkHeader->dwUnknownSize); */
    ...
    if(Header.dwID == ID_MPK && Header.dwHeaderSize == sizeof(TMPKHeader) && Header.dwArchiveSize == (uint32_t)FileSize)
    {
...
static TMPQHash * FindFreeHashEntry2(TMPQHash * pHashTable, uint32_t dwHashTableSize, uint32_t dwStartIndex)
{
    TMPQHash * pHash;
    uint32_t dwIndex;
...
{
    unsigned char * pbMpkTable = (unsigned char *)pvMpkTable;
    size_t i;
...
void * LoadMpkTable(TMPQArchive * ha, uint32_t dwByteOffset, uint32_t cbTableSize)
{
    uint64_t ByteOffset;
    unsigned char * pbMpkTable = NULL;

    /* Allocate space for the table */
    pbMpkTable = STORM_ALLOC(uint8_t, cbTableSize);
    if(pbMpkTable != NULL)
    {
...
TMPQHash * LoadMpkHashTable(TMPQArchive * ha)
{
    ...
    TMPKHash * pMpkHash;
    TMPQHash * pHash = NULL;
    uint32_t dwHashTableSize = pHeader->dwHashTableSize;
    ...
    if(pMpkHash != NULL)
    {
        ...
        if(pHashTable != NULL)
        {
            ...
            for(i = 0; i < dwHashTableSize; i++)
            {
                ...
                pHash = FindFreeHashEntry2(pHashTable, pHeader->dwHashTableSize, pMpkHash[i].dwName1);
...
static uint32_t ConvertMpkFlagsToMpqFlags(uint32_t dwMpkFlags)
{
    uint32_t dwMpqFlags = MPQ_FILE_EXISTS;
