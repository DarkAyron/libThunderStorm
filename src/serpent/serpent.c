		     ...
		     unsigned int keylen)
{
	...
	r0 = BSWAP_INT32_UNSIGNED(k[3]);
	r1 = BSWAP_INT32_UNSIGNED(k[4]);
	r2 = BSWAP_INT32_UNSIGNED(k[5]);
	r3 = BSWAP_INT32_UNSIGNED(k[6]);
	r4 = BSWAP_INT32_UNSIGNED(k[7]);

	keyiter(BSWAP_INT32_UNSIGNED(k[0]), r0, r4, r2, 0, 0);
	keyiter(BSWAP_INT32_UNSIGNED(k[1]), r1, r0, r3, 1, 1);
	keyiter(BSWAP_INT32_UNSIGNED(k[2]), r2, r1, r4, 2, 2);
	keyiter(BSWAP_INT32_UNSIGNED(k[3]), r3, r2, r0, 3, 3);
	keyiter(BSWAP_INT32_UNSIGNED(k[4]), r4, r3, r1, 4, 4);
	keyiter(BSWAP_INT32_UNSIGNED(k[5]), r0, r4, r2, 5, 5);
	keyiter(BSWAP_INT32_UNSIGNED(k[6]), r1, r0, r3, 6, 6);
	keyiter(BSWAP_INT32_UNSIGNED(k[7]), r2, r1, r4, 7, 7);
...
/*
 * Note: The conversions between uint8_t* and uint32_t* might cause trouble
 * on architectures with stricter alignment rules than x86
 */

	r0 = BSWAP_INT32_UNSIGNED(s[0]);
	r1 = BSWAP_INT32_UNSIGNED(s[1]);
	r2 = BSWAP_INT32_UNSIGNED(s[2]);
	r3 = BSWAP_INT32_UNSIGNED(s[3]);
	...
	d[0] = BSWAP_INT32_UNSIGNED(r0);
	d[1] = BSWAP_INT32_UNSIGNED(r1);
	d[2] = BSWAP_INT32_UNSIGNED(r2);
	d[3] = BSWAP_INT32_UNSIGNED(r3);
}
...
void serpentDecrypt(struct serpentSchedule *ctx, uint8_t *src, const uint8_t *dst)
{
	...
	d[0] = BSWAP_INT32_UNSIGNED(r2);
	d[1] = BSWAP_INT32_UNSIGNED(r3);
	d[2] = BSWAP_INT32_UNSIGNED(r1);
	d[3] = BSWAP_INT32_UNSIGNED(r4);
}
